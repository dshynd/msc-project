...
import uk.ac.stand.cs.insense.compiler.barrelfishCCgen.fileHandling.HeaderFile;
import uk.ac.stand.cs.insense.compiler.barrelfishCCgen.fileHandling.ImplFile;
...
public abstract class StructDeclaration extends Code implements IStructDeclaration {
	...
	private StringBuffer arrayHeaderBuffer = new StringBuffer();
	 ...
	 * 
	 * @see uk.ac.stand.cs.insense.compiler.barrelfishCCgen.Code#complete()
	 */
	...
	private StringBuffer getArraySourceCodeBuffer() {
		StringBuffer result = arrayBuffer;
		arrayBuffer = new StringBuffer();
		return result;
	}
	
	private StringBuffer getArrayHeaderDeclBuffer() {
		StringBuffer result = arrayHeaderBuffer;
		arrayHeaderBuffer = new StringBuffer();
		return result;
	}
	...
	private void generateArrayCopyFunction(ArrayType at) {
		if (!ArrayConstructor.generatedConstructorAlready(at)) {
			...
			arrayHeaderBuffer.append(EXTERN_ + ArrayConstructor.constructorSignature(at) + SEMI + NEWLINE);
		}
		if (!ArrayConstructor.generatedCopyfunctionAlready(at)) {
			arrayBuffer.append(ArrayConstructor.copyFunctionCode(at));
			arrayHeaderBuffer.append(EXTERN_+ ArrayConstructor.copyFunctionSignature(at) + SEMI + NEWLINE);
		}
	}
	...
	private String decRefPrototype() {
		return STATIC_ + VOID_ + "decRef_" + struct_name() + LRB_ + struct_pntr_name() + "pntr" + SPACE + RRB;
	}
	...
	private void printDOTHHeaders(PrintStream ps) {
		...
		ps.println("#include <" + LIBRARY_NAME + "/InsenseRuntime.h>");
		ps.println("#include <" + LIBRARY_NAME + "/IString.h>");
		ps.println("#include <" + LIBRARY_NAME + "/AnyType.h>");
	...
	private void generateSourceFile() {
		try {
			...
			String arrayFunctions = getArraySourceCodeBuffer().toString() + "\n";
	...
	private void generateHeaderFile() {
		try {
			...
			String arrayFunctionDecls = getArrayHeaderDeclBuffer().toString() + "\n";
			ps.print(arrayFunctionDecls);
