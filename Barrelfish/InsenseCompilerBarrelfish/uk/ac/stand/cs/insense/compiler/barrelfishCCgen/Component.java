...
public class Component extends ProcedureContainer implements ICode, IComponent {
	...
	private void printCommonDecls(PrintStream ps) {
		...
		ps.print(TAB + "my_sem_t" + SPACE + "component_create_sem" + SEMI + NEWLINE);
	...
	private void printImplIncludes(PrintStream ps) {
		...
		ps.println(HASH_INCLUDE_ + LAB + LIBRARY_NAME + "/" + "my_semaphore.h" + RAB);
	...
	private String generateDecRef() {
		...
		sb.append("// TODO Safe GC for components not implemented yet ... this function will not be called" + NEWLINE);
	...
	private void printConstructorGlobalInitialiser(PrintStream ps) {
		ps.println(GENERATED_FROM + Diagnostic.getMethodInCallChain());
		ps.println(STATIC_ + VOID_ + initGlobalsName + LRB_ + data_pntr_name(componentname) + THIS_ + RRB_);
		ps.println(LCB_);
